// cv0conv.cpp : Defines the entry point for the console application.
//
...
void process_file (std::string file_name)
{
	...
	file_name = file_name.substr(0, file_name.length() - 3) + "txt";
...
void find_file (std::string path)
{
	...
	HANDLE hFind = FindFirstFile ((path + "*.cv0").c_str(), &FindFileData);
