// convcv0.cpp : Defines the entry point for the console application.
//
...
void process_file (std::string input)
{
	...
	std::string output = input.substr(0, input.length() - 3) + "cv0";
...
void find_file (std::string path)
{
	...
	HANDLE hFind = FindFirstFile ((path + "*.txt").c_str(), &FindFileData);
